...
We used this tool because we wanted to check if data could survive a cleaning tool. Since we couldn't test over longer periods of time, and since the phone had very low memory, we used a cleaningtool to mimmic a environment for safety concious people. based on what discussed in the section \ref{Background} about phone memory, it would be possible to get more data from the memory dump with a phone with more memory space than 1bg. The average phone today have around 4gb of mamory and would possibly provide us with more data and more data over a loger period of time.\\\\
